...
int sht_hash_function(char surname[MAX_BYTES] , int buckets) {
  
  int sum = 0;
  for(int i = 0; i < MAX_BYTES; i++){

    if(surname[i] == '\0')    // End of the string
      break;

    sum += (int)surname[i];   // Add the ASCII code of each character
  }
  ...
  void access_hashtable_block(HT_info* ht_info, int wanted_block) {

    BF_Block* block;
    BF_Block_Init(&block);

    BF_GetBlock(ht_info->file_desc,wanted_block,block);
...
int SHT_SecondaryInsertEntry(SHT_info* sht_info, Record record, int block_id){
  ...
  int bucket = sht_hash_function(record.surname, sht_info->buckets);
...
int SHT_SecondaryGetAllEntries(HT_info* ht_info, SHT_info* sht_info, char* name){
  ...
  do{
    ...
    for(int i = 0; i < records; i++){
      ...
      // If the surname of the record we are accessing is the same as wanted,
      // we have found the block_id of the hashtable
      if(strcmp(cur_data[i].surname,name))
        wanted_block = cur_data[i].block_id;
        access_hashtable_block(ht_info, wanted_block);
    }
...
int SHTStatistics(char* filename){
